...
void astree::print (FILE* outfile, astree* tree, int depth) {
   ...
   for (astree* child: tree->children) {
     for(int i = 1; i <= depth+1; i++) fprintf(outfile, "%s%*s", "|", 3, "");
      astree::print (outfile, child, depth + 1);
   }
}

/*void astree::tok_print (FILE* outfile, astree* tree, int depth) {
   fprintf(outfile,"%zd%2zd.%zd%2zd %2s%3s",
    tree->lloc.filenr, tree->lloc.linenr, tree->lloc.offset,
        tree->symbol,parser::get_tname(tree->symbol),yytext);
